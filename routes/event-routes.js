...
moment().format(); 
// middleware to chech if user is logges in
...
router.post('/create',[
    ...
    check('date').isLength({min:5}).withMessage('Date must be valid date'),
],(req,res)=>{
...
router.get('/:pageNo?',(req,res)=>{
    ...
    let totalDocs = 0;
    Event.countDocuments({}).then((res)=>{
        //console.log(res)
        totalDocs = parseInt(res)
    })
    // end of countDoc
